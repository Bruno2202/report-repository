...
const SelectFiles: React.FC<SelectFilesProps> = ({
    ...
    async function handleOpenConfirmDelete(filename: string) {
        setFilename(filename);
        openModal("ConfirmDeleteFile")
    }
    ...
    return (
        <>
                    ...
                    <div className="flex flex-col gap-2">
                        {report.xml && !xml ? (
                            <div className="flex items-center justify-between w-full h-28 px-4 border-2 border-blue/20 bg-blue/5 rounded-xl">
                                ...
                                <button
                                    onClick={() => handleOpenConfirmDelete(report.xml!)}
            ...
            {isOpenModal("ConfirmDeleteFile") &&
                <ConfirmDelete
                    deleteFunction={handleDeletePhysicalFile}
                    title="Arquivo"
                    dataTile={filename}
                    loading={deleting}
                    modalName="ConfirmDeleteFile"
                />
            }
        </>
    );
